...
transformed parameters{
  ...
  delta_x_2[1] =  x[1, 12] - x0[12] + 1e-5;
  for (i in 1:n_obs-1) {
    delta_x_1[i + 1] = x[i + 1, 11] - x[i, 11] + 1e-5;
    delta_x_2[i + 1] = x[i + 1, 12] - x[i, 12] + 1e-5;
  }
}
model {
  CF ~ beta(2, 2);
  ...
  H ~ neg_binomial_2(delta_x_1, phi2);
  D ~ neg_binomial_2(delta_x_2, phi3);
}
generated quantities {
  real log_lik;
  array[n_obs] int sim_H;
  array[n_obs] int sim_D;
  log_lik = neg_binomial_2_lpmf(H | delta_x_1, phi2)+neg_binomial_2_lpmf(D | delta_x_2, phi3);
  sim_H = neg_binomial_2_rng(delta_x_1, phi2);
  sim_D = neg_binomial_2_rng(delta_x_2, phi3);
}
